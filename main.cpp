...
//  Vaxman_EA
//
//  Created by Efe Osah on 2021-07-14.
//
...
//create vectors to keep track of monsters/ghosts
vector<float> monsterCoordinates1 = {10.5, 13.5, 4.5, 2.5};
vector<float> monsterCoordinates2 = {8.5, 1.5, 6.5, 13.5};
vector<float> monsterDirection = {1.0, 2.0, 3.0, 4.0};
...
//ghosts.push_back(monster1); //{monster1, monster2, monster3, monster4};
...
void updateMonster(){
    ...
    for(int i = 0; i < monsterCoordinates1.size(); ++i)
    {
        float monster[3] = {monsterCoordinates1[i], monsterCoordinates2[i], monsterDirection[i]};
        ...
        switch ((int)monsterDirection[i]){
        case 1:
            if (!bitmap.at(x1Quadrant).at((int)monsterCoordinates2[i])){
                monsterCoordinates1[i] -= 2 / squareSize;
            }else {
                int current = monsterDirection[i];
                do{
                    monsterDirection[i] =  (rand() % 4) + 1;
                } while (current == (int) monsterDirection[i]);
            }
            break;
        case 2:
            if (!bitmap.at(x2Quadrant).at((int)monsterCoordinates2[i])){
                monsterCoordinates1[i] += 2 / squareSize;
            }
            else {
                int current = monsterDirection[i];
            ...
            break;
        case 3:
            if (!bitmap.at((int)monsterCoordinates1[i]).at(y1Quadrant)){
                monsterCoordinates2[i] -= 2 / squareSize;
            }
            else {
                int current = monsterDirection[i];
                do{
            ...
            break;
        case 4:
            if (!bitmap.at((int)monsterCoordinates1[i]).at(y2Quadrant)){
                monsterCoordinates2[i] += 2 / squareSize;
            }
            else {
                int current = monsterDirection[i];
...
//Method to duplicate remaining ghosts
void duplicateMonster(){
    
    //glutPostRedisplay();
    //glutTimerFunc(300, duplicateMonster, 0);
    int remainingGhosts = monsterCoordinates1.size();
    printf("\n\n\n check\n\n\n");
    //display();
    for(int i = 0; i < remainingGhosts; i++)
    {
        ...
        if(randomNum == 1)
        {
            monsterCoordinates1.push_back(10.5);
            monsterCoordinates2.push_back(8.5);
            monsterDirection.push_back(1.0);
        }
        else if(randomNum == 2)
        {
            monsterCoordinates1.push_back(13.5);
            monsterCoordinates2.push_back(1.5);
            monsterDirection.push_back(2.0);
        }
        else if(randomNum == 3)
        {
            monsterCoordinates1.push_back(4.5);
            monsterCoordinates2.push_back(6.5);
            monsterDirection.push_back(3.0);
        }
        else if(randomNum == 4)
        {
            monsterCoordinates1.push_back(2.5);
            monsterCoordinates2.push_back(13.5);
            monsterDirection.push_back(4.0);
        }
    }
...
void resetGame(){
    ...
    yIncrement = 0;
    rotation = 0;
    monsterCoordinates1.clear();
    monsterCoordinates2.clear();
    monsterDirection.clear();
    
    monsterCoordinates1 = {10.5, 13.5, 4.5, 2.5};
    monsterCoordinates2 = {8.5, 1.5, 6.5, 13.5};
    monsterDirection = {1.0, 2.0, 3.0, 4.0};
    /*
...
void gameOver(){
    ...
    for(int i = 0; i < monsterDirection.size(); ++i)
    {
        int monsterX = (int)(monsterCoordinates1[i]);
        int monsterY = (int)(monsterCoordinates2[i]);
        
        if(pacmanX == monsterX && pacmanY == monsterY)
        {
            monsterCoordinates1.erase(monsterCoordinates1.begin()+ i);
            monsterCoordinates2.erase(monsterCoordinates2.begin()+ i);
            monsterDirection.erase(monsterDirection.begin()+ i);
            
        }
    }
    
    /*
    //remove ghost if the come in contact with vaxman
...
void welcomeScreen(){
    ...
    while (*message)
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, *message++);
    message = "VAXMAN - by Efe Osah for EA Sports";
    glColor3f(1, 1, 1);
...
void display(){
    ...
    if (replay){
        if (!over){
            ...
            for(int i = 0; i < monsterCoordinates1.size(); ++i)
            {
                float temp[3] = {monsterCoordinates1[i], monsterCoordinates2[i], monsterDirection[i]};
                updateMonster(temp , i+1);
            ...
            for(int i = 1; i <= monsterCoordinates1.size(); ++i)
            {
                
                
                if(i % 2 == 0)
                {
                    drawMonster(monsterCoordinates1[i-1], monsterCoordinates2[i-1], 1.0, 0.0, 0.0);
                }
                else if(i % 3 == 0)
                {
                    drawMonster(monsterCoordinates1[i-1], monsterCoordinates2[i-1], 1.0, 0.0, 0.6);
                }
                else if(i % 4 == 0)
                {
                    drawMonster(monsterCoordinates1[i-1], monsterCoordinates2[i-1], 1.0, 0.3, 0.6);
                }
                else if(i % 1 == 0)
                {
                    drawMonster(monsterCoordinates1[i-1], monsterCoordinates2[i-1], 0.0, 1.0, 1.0);
                }
...
int main(int argc, char** argv){
    ...
    glutCreateWindow("VAXMAN - by Efe Osah");
    ...
    //run the game
    init();
    //glutTimerFunc(3000.0, updateMonster(), 0);
